...
module.exports = joinhandler = async (data, sock) => {
	...
	const gMi = data.action === "remove" ? await sock.groupMetadata(data.id) : "";
	const myID =
		sock.type === "legacy"
	...
	if (data.action === "add" && !data.participants.includes(myID)) {
		...
		if (info !== "no_file") {
			...
			if (dataConf["join"]["active"]) {
				await sock.sendMessage(data.id, { image: { url: 'https://telegra.ph/file/a608c88cd5a9d280ecff7.png' }, caption: text, mentions: data.participants});
			}
		}
	} else if (data.action === "remove" && !data.participants.includes(myID)) {
		let id = data.id.split("@")[0];
