...
function printLog(isCmd, sender, gcName, isGc, msg) {
	if (isCmd && isGc) {
		return console.log(color("[EXEC]", "aqua"), color(sender.split("@")[0], "lime"), "in", color(gcName, "lime"), color(msg.body, "yellow"));
	}
	if (isCmd && !isGc) {
		return console.log(color("[EXEC]", "aqua"), color(sender.split("@")[0], "lime"), color(msg.body, "yellow"));
	}
}
...
module.exports = chatHandler = async (m, sock) => {
	try {
		...
		// Log
		printLog(isCmd, sender, gcName, isGroup, msg);
		...
		if (cmd.premium && !isOwner) {
			...
			if (!cmd.premiumType.includes(userData.type))
				return await msg.reply(`This command only for premium _'${cmd.premiumType.join(" / ")}'_\nMakanya bayar !`);
		}
